...
void find_intermediate_key_solution(uint8_t** keys, int number_of_keys, uint8_t* chosen_valid_key) {
    srand(time(NULL));
    uint8_t* tweak1 = malloc(16);
    uint8_t* tweak2 = malloc(16);
    uint8_t tweak_difference = (uint8_t)(rand() % 256);

    for (int i = 0; i < 16; i++) {
        tweak1[i] = (uint8_t)(rand() % 256);
    }
...
void attack_byte(int column, int byte_number, int start_side, int key_number) {
    ...
    */
    ...
    tweak2[4*column+byte_number] = tweak1[4*column+byte_number] ^ tweak_difference;
    ...
    for (int possible_c1_value = 0; possible_c1_value < 256; possible_c1_value++) {
        ...
        mixColumns(c1_tilde);
        add(c1_tilde, tweak1);
        compute_sibling(c1_tilde, key, tweak1, start_side);
        add(c1_tilde, tweak1);
        inverseMixedColumn(c1_tilde);
        inv_shift_rows(c1_tilde);
    ...
    for (int possible_c1_value = 0; possible_c1_value < 256; possible_c1_value++) {
        ...
        mixColumns(c1_tilde);
        add(c1_tilde, tweak2);
        compute_sibling(c1_tilde, key, tweak2, start_side);
        add(c1_tilde, tweak2);
        inverseMixedColumn(c1_tilde);
        inv_shift_rows(c1_tilde);
